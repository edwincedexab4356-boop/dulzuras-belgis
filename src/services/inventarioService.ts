...
export const inventarioService = {
    ...
    if (isFirebaseConfigured() && db) {
      try {
        ...
        unsubFirestore = onSnapshot(
          ...
          (error) => {
            console.warn('Error onSnapshot inventario_movimientos, using fallback:', error?.message || error);
            callback(getLocalMovimientos());
          }
        );
      } catch (err) {
  ...
  async sincronizarInventarioConFirestore(productos: Producto[]): Promise<void> {
    if (!isFirebaseConfigured() || !db || !productos || productos.length === 0) return;
    try {
      ...
      console.warn('Error sincronizando inventario con Firestore:', e);
    }
  },
  ...
  async registrarMovimiento(mov: Omit<MovimientoInventario, 'id'>): Promise<MovimientoInventario> {
    ...
    if (isFirebaseConfigured() && db) {
      try {
        ...
        // Sync directly to the 'inventario' collection in Firestore
        if (mov.productoId) {
          try {
            await setDoc(
              doc(db, 'inventario', String(mov.productoId)),
              {
                id: String(mov.productoId),
                productoId: String(mov.productoId),
                nombre: prodName,
                stock: cantNue,
                stockAnterior: cantAnt,
                diferencia: diff,
                ultimoTipoMovimiento: mov.tipo || (diff >= 0 ? 'entrada' : 'salida'),
                ultimoMotivo: mov.motivo || 'Movimiento de inventario',
                usuario: userName,
                actualizadoEn: now,
              },
              { merge: true }
