...
export const authService = {
  ...
  /**
   * Real Email + Password authentication with optional shift cashier name.
   */
  async login(email: string, password: string, cajeroJornada?: string): Promise<UserAuth> {
    ...
    if (isFirebaseConfigured() && auth) {
      try {
        ...
        const userAuth: UserAuth = {
          uid: u.uid,
          email: u.email,
          displayName: cajeroJornada?.trim() || u.displayName || (u.email ? u.email.split('@')[0] : 'Usuario'),
          role,
    ...
    const localUser: UserAuth = {
      ...
      email: cleanEmail,
      displayName: cajeroJornada?.trim() || cleanEmail.split('@')[0],
      role: cajeroJornada ? 'cajero' : role,
      cajeroJornada: cajeroJornada?.trim() || undefined,
    };
