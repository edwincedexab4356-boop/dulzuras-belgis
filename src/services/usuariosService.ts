...
const DEFAULT_USUARIOS: UsuarioDoc[] = [
  {
    id: 'HqXvFFCCQGaovRbdosePTUtqoR23',
    uid: 'HqXvFFCCQGaovRbdosePTUtqoR23',
  ...
  {
    id: 'GvojIR39cvlfdofv1Sg9',
    uid: 'oPpdvflnLcf4GXL5cgsMaXhbA583',
    ...
    if (isFirebaseConfigured() && db) {
      try {
        ...
        const isKnownAdminEmail =
          cleanEmail === 'pedro76porro@gmail.com' ||
        ...
        const isKnownCajeroEmail =
          cleanEmail === 'edwincede.xab4356@gmail.com' ||
    ...
    if (isFirebaseConfigured() && db && auth?.currentUser) {
      try {
        ...
        unsubFirestore = onSnapshot(
          ...
          (err: Error) => {
            console.warn('Notice on snapshot usuarios:', err?.message || err);
            callback(getLocalUsuarios());
          }
        );
      } catch (e) {
